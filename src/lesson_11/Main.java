...
/*        Создать класс, в котором будет статический метод. Этот метод принимает на вход три
        ...
        login = Authorization.scan().get();
        System.out.println("Введите пароль");
        password = Authorization.scan().get();
        System.out.println("Повторите пароль");
        confirmPassword = Authorization.scan().get();

        try {
            Authorization.checkAuth(login, password, confirmPassword);
            System.out.println("Поздравляем, Вы успешно авторизировались!");
